...
def test_modes_grab(tk_window, mode):
    with sh.WindowCapture(hwnd=tk_window.hwnd, mode=mode, cursor=False, border=False) as cap:
...
def test_bad_mode(tk_window):
    with pytest.raises(ValueError):
        sh.WindowCapture(hwnd=tk_window.hwnd, mode="turbo")
...
def test_static_window_does_not_time_out(tk_window):
    ...
    with sh.WindowCapture(hwnd=tk_window.hwnd, timeout_ms=100) as cap:
        cap.grab()
        time.sleep(1.0)
...
def test_live_mode_is_fast(tk_window):
    with sh.WindowCapture(hwnd=tk_window.hwnd, mode="live") as cap:
        cap.grab()
        t0 = time.perf_counter()
        for _ in range(20):
            cap.grab()
